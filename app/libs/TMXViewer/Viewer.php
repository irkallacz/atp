...
class Viewer {
		...
		foreach($this->map->tilesets as $i=>$ts) {
			if( strlen($ts->source)==0 ) {
				...
				foreach($ts->tiles as $id => $ar) {
					if(is_int($id)) {
						if(array_key_exists('imagecontent', $ar)) {
							...
							$this->ts_imgs[$i][$id]=Functions::create_image_from(dirname($this->map->filename).'/'.$ar['imagesource']);
						}
						else {
				...
				if(array_key_exists($_REQUEST['ref'], Viewer::$urls)) {
					...
					$url.=$ts->source;
					$this->ts_imgs[$i]=Functions::create_image_from($url);
				}
				else {
					$this->ts_imgs[$i]=Functions::create_image_from(dirname($this->map->filename).'/'.dirname($ts->sourceTSX).'/'.$ts->source);
				}
			}
			else {
				//$this->ts_imgs[$i]=Functions::create_image_from(dirname($this->map->filename).'/'.dirname($ts->sourceTSX).'/'.$ts->source);
				$dir = dirname($this->map->filename);
				//str_replace('/res/','')
				$dir = strtr(dirname($this->map->filename),['res' => 'www/drawable', 'xml' => '']);
				$this->ts_imgs[$i]=Functions::create_image_from($dir.basename($ts->source));
			}
			...
			if((bool)$transc && $transc!='') {
				...
				Functions::my_transparent($this->ts_imgs[$i], $r, $g, $b, $trans);
	...
	private function draw_tile(&$ly, $cgid, $i=NULL, $j=NULL, &$o=NULL) {
		...
		if(!is_object($o)) {
			if(strlen($this->map->tilesets[$ti]->name)>0&&in_array($this->map->tilesets[$ti]->name, $this->tilesets_nodraw)) return;
		}
		...
		if($this->map->orientation=='orthogonal') {
			...
			if($this->zoom==1) {
				Functions::image_copy_and_resize($this->img, $tsimg, $this->ox+$dx, $this->oy+$dy, $sx, $sy, $sw, $sh);
			}
			else {
				Functions::image_copy_and_resize($this->img, $tsimg, $this->ox+$dx*$this->zoom, $this->oy+$dy*$this->zoom, $sx, $sy, $sw*$this->zoom, $sh*$this->zoom, $sw, $sh);
			}
	...
	*/
	...
	public function draw_tilelayer($tl, $x=0, $y=0, $w=PHP_INT_MAX, $h=PHP_INT_MAX) {
		...
		if(strlen($tl->name)>0&&in_array($tl->name, $this->layers_nodraw)) return;
	...
	public function draw_layers($x=0, $y=0, $w=PHP_INT_MAX, $h=PHP_INT_MAX) {
		...
		foreach($this->map->layers as $index=>$ly) {
			...
			elseif($ly instanceof ImageLayer) {
				if($this->draw_images) {
					$il=$ly;
					$img_=Functions::create_image_from(dirname($this->map->filename).'/'.$il->source);
					...
					if((bool)$transc && $transc!='') {
						...
						Functions::my_transparent($img_, $r, $g, $b, $trans);
					...
					if($this->map->orientation=='orthogonal') {
						...
						if($this->zoom==1) {
							Functions::image_copy_and_resize($this->img, $img_, $this->ox+$il->x, $this->oy+$il->y, 0, 0, $sw, $sh);
						}
						else {
							Functions::image_copy_and_resize($this->img, $img_, $this->ox+$il->x*$this->zoom, $this->oy+$il->y*$this->zoom, 0, 0, $sw*$this->zoom, $sh*$this->zoom, $sw, $sh);
						}
					}
					elseif($this->map->orientation=='isometric') {
