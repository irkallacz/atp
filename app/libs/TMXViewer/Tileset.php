				...
				if((bool)$tile->image!=false) {
					...
					if(strlen($this->tiles[(string)$tile['id']]['imagecontent'])>0) {
						if( strlen($this->tiles[(string)$tile['id']]['imageencoding'])>0 && strlen($this->tiles[(string)$tile['id']]['imagecompression'])>0 ) {
							$this->tiles[(string)$tile['id']]['imagecontent']=Functions::parse_data($this->tiles[(string)$tile['id']]['imagecontent'], $this->tiles[(string)$tile['id']]['imageencoding'], $this->tiles[(string)$tile['id']]['imagecompression']);
						}
					}
